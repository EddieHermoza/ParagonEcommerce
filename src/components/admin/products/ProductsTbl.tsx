				...
				<table className="table-auto text-center w-full ">
					<thead className=" border-b-primary border-b relative text-sm lg:text-base ">
						<tr className="h-16">
							...
							<td className="max-sm:hidden">
								<button onClick={() => handleSort('price')} className='flex-center gap-2 mx-auto active:bg-accent hover:bg-hover p-2 rounded'>
									<HiOutlineArrowsUpDown />
									Precio
								</button>
							</td>
					...
					<tbody className="text-sm relative">
						{sortedProducts.map((product, index) => (
							<tr key={index} className="hover:bg-muted/50 duration-200 relative h-24">
								...
								<td className=
									{`max-sm:hidden text-shadow-lg ${product.status === 1 ? 'text-green-500 shadow-green-500/50' : 'text-red-500 shadow-red-500/50'
										}`}
								>
								...
								<Popover>
										...
										<PopoverContent align="end" className="flex flex-col gap-2 items-start text-sm w-auto " sideOffset={6}>
											...
											<Link href={`/admin/products/edit/${product.id}`} className="flex items-center gap-2 hover:bg-muted/50 p-2 w-full rounded-sm ">
												<FiEdit size={18} /> Editar
											</Link>
											<button className="flex items-center gap-2 hover:bg-muted/50 p-2 rounded-sm w-full">
